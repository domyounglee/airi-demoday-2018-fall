...
<body>
   <header>
      ...
      <div class="navbar navbar-dark bg-dark box-shadow">
        <div class="container d-flex justify-content-between">
          <a href="#" class="navbar-brand d-flex align-items-center">
            <strong>Genesis Manual Chatbot</strong>
          </a>
      	  <span id="fixed" class="navbar-toggler-icon" href="#"><img src="./AIRI_logo.png" width="90px" height="53px"/></a></span>
        </div>
      </div>
    </header>
    <section class="jumbotron text-center" style="padding: 1rem 0rem;">
		...
		안녕하세요. 제네시스 챗봇 매뉴얼 입니다.
	  </h1>
          <p class="lead text-muted">
		<img src="./Genesis.png" class="img-fluid" alt="Responsive image" style="width:300px;" />
	  </p>
        </div>
    </section>
	...
	<div class="row">
		...
		<div class="card">
  			<div class="card-body" style="height: 500px;">
				<img id="G-manual-img" src="../img/default_genesis.jpeg" class="img-fluid" alt="Responsive image">
			</div>
		</div>
    	</div>
      	</div>
	<div class="row" style="visibility:hidden;">
		<form id="frmChat" action="#" > <!-- start FORM -->
     			...
     			<div class="formRow"> <!-- start FORM ROW 3 -->
     				<h4>대화:</h4>
     				<input type="text" name="message" id="GenesisTM"/>
    ...
    <script type="text/javascript">
        function sendToParent(message){
     		window.opener.document.getElementById("SantafeTM").value = message;
		window.opener.callSantafeResult();
	}
	...
	$(function(){
		$('#frmChat').submit(function(e){
			...
			sendToParent(document.getElementById('SantafeTM').value);
		});
		
	});

        function callGenesisResult(){
        	//e.preventDefault();  // Prevent the default submit() method
        	...
        	var youSaid = '<font size=5>' + '<strong>' + name + ':</strong> ' + $('#GenesisTM').val() + '</font>' + "<br>\n";
        	setTimeout(function(){update(youSaid)}, 2500);
	...
	$.ajax({
		...
		setTimeout(function(){processResponse(parseCommands(response))}, 5000);
	...
	function processResponse(response) { // given the final CS text, converts the parsed response from the CS server into HTML code for adding to the response holder div
		...
		if (response['img'] !=null ){
			  var man=window.opener.document.getElementById("S-manual-img");
			  man.src='../img/pages/g/' + response['img'] + '.png'; 
		}
                else
                        $('#G-manual-img').attr('src','../img/default_genesis.jpeg');
                update(botSaid);
                speak(response['text']);
                setTimeout(function(){sendToParent(response['text'])},2500)
        }
